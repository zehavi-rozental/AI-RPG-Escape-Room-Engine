...
function GameCard({ game }: GameCardProps) {
  return (
    <div className="group relative flex flex-col overflow-hidden rounded-xl border border-amber-900/40 bg-gradient-to-b from-[#1c1722] to-[#0c0a10] shadow-lg shadow-black/60 transition-all duration-300 hover:-translate-y-1.5 hover:border-amber-500/70 hover:shadow-2xl hover:shadow-amber-900/30">
      ...
      <div className="flex flex-1 flex-col gap-3 p-5 text-right">
        ...
        <Link
          to={`/games/${game._id}`}
          className="mt-2 block w-full rounded-lg border border-amber-600/50 bg-gradient-to-r from-amber-700 via-orange-700 to-red-800 py-2.5 text-center font-bold text-amber-50 shadow-md transition-all duration-300 hover:-translate-y-0.5 hover:shadow-[0_0_22px_rgba(245,158,11,0.6)] hover:brightness-110 active:scale-[0.98]"
        >
          🔒 לכניסה למשחק
        </Link>
      </div>
