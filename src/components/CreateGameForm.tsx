...
const DIFFICULTY_OPTIONS: { value: Difficulty; color: string }[] = [
  { value: "Easy",   color: "text-emerald-400 border-emerald-400/40 hover:border-emerald-400" },
  { value: "Medium", color: "text-amber-400  border-amber-400/40  hover:border-amber-400"  },
  { value: "Hard",   color: "text-rose-400   border-rose-400/40   hover:border-rose-400"   },
];
    ...
    return (
      <div className="min-h-screen bg-neutral-950 flex items-center justify-center p-6">
        <div className="text-center space-y-6">
          ...
          <h2 className="text-3xl text-amber-400 tracking-widest" style={{ fontFamily: "'Cinzel', serif" }}>
            ROOM FORGED
          </h2>
          <p className="text-neutral-400 text-sm font-mono">Your escape room has been created.</p>
          <button
  ...
  return (
    <div className="min-h-screen bg-neutral-950 relative flex items-center justify-center px-4 py-16 overflow-hidden">
      <div className="pointer-events-none absolute inset-0">
        ...
        <div className="absolute inset-0 opacity-[0.04]" style={{ backgroundImage: "linear-gradient(rgba(251,191,36,0.3) 1px, transparent 1px), linear-gradient(90deg, rgba(251,191,36,0.3) 1px, transparent 1px)", backgroundSize: "40px 40px" }} />
      ...
      <div className="relative z-10 w-full max-w-2xl">
        <div className="mb-10 text-center">
          <div className="inline-flex items-center gap-2 mb-4 px-4 py-1.5 border border-amber-400/20 rounded-full bg-amber-400/5">
            ...
            <span className="text-amber-400/60 text-xs font-mono tracking-widest uppercase">Game Master Console</span>
          </div>
          <h1 className="text-4xl md:text-5xl font-black tracking-tight text-white" style={{ fontFamily: "'Cinzel', 'Georgia', serif", letterSpacing: "0.05em" }}>
            FORGE A NEW
            <span className="block text-transparent bg-clip-text bg-gradient-to-r from-amber-400 to-orange-500">
              ESCAPE ROOM
            </span>
          </h1>
          <p className="mt-3 text-neutral-500 text-sm font-mono">Design the labyrinth. Set the trap.</p>
        </div>

        <form onSubmit={handleSubmit(onSubmit)} noValidate className="relative rounded-2xl overflow-hidden border border-neutral-800 bg-neutral-900/80 backdrop-blur-xl shadow-[0_0_60px_rgba(0,0,0,0.6),inset_0_1px_0_rgba(255,255,255,0.04)]">
          ...
          <div className="p-8 md:p-10 space-y-8">
            ...
            <div>
              ...
              <input
                {...register("title", { required: "A title is required.", minLength: { value: 3, message: "At least 3 characters." }, maxLength: { value: 80, message: "Maximum 80 characters." } })}
                type="text"
            ...
            <div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                {GENRE_OPTIONS.map(({ value, label, icon }) => {
                  ...
                  return (
                    <button key={value} type="button" onClick={() => setValue("genre", value, { shouldValidate: true })}
                      ...
                      {label}
                      {active && <span className="absolute top-1.5 right-1.5 w-1.5 h-1.5 rounded-full bg-amber-400" />}
                    </button>
                  );
                })}
              </div>
            ...
            <div>
              ...
              <div className="grid grid-cols-3 gap-3">
                {DIFFICULTY_OPTIONS.map(({ value, color }) => {
                  ...
                  return (
                    <button key={value} type="button" onClick={() => setValue("difficulty", value, { shouldValidate: true })}
            ...
            <div className="grid sm:grid-cols-2 gap-6">
              <div>
                ...
                <input type="hidden" {...register("targetAudience", { required: true })} />
                <div className="flex flex-col gap-2">
                  {AUDIENCE_OPTIONS.map((value) => {
                    const active = selectedAudience === value;
                    return (
                      <button key={value} type="button" onClick={() => setValue("targetAudience", value, { shouldValidate: true })}
                        ...
                        <span className={`w-2 h-2 rounded-full border transition-colors ${active ? "bg-amber-400 border-amber-400" : "border-neutral-600"}`} />
                        {value}
                      </button>
                    );
                  })}
              ...
              <div>
                ...
                <div className="relative">
                  <input
                    {...register("playerCount", { required: "Required.", min: { value: 1, message: "Minimum 1 player." }, max: { value: 20, message: "Maximum 20 players." }, valueAsNumber: true })}
                  ...
                  <div className="absolute right-3 inset-y-0 flex flex-col justify-center gap-1">
                    <button type="button" onClick={() => { const cur = watch("playerCount"); if (cur < 20) setValue("playerCount", cur + 1); }} className="text-neutral-500 hover:text-amber-400 transition-colors leading-none text-xs">▲</button>
                    <button type="button" onClick={() => { const cur = watch("playerCount"); if (cur > 1) setValue("playerCount", cur - 1); }} className="text-neutral-500 hover:text-amber-400 transition-colors leading-none text-xs">▼</button>
                  </div>
                </div>
                <p className="mt-2 text-neutral-600 text-xs font-mono text-center">1 – 20 players</p>
        ...
        <p className="mt-6 text-center text-neutral-700 text-xs font-mono">Only Game Masters can forge rooms. Play wisely.</p>
      </div>
    </div>
  );
}